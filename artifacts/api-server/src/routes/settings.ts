...
const settingsSchema = z.object({
  ...
  invoiceOverdueEmails: z.boolean().default(true),
  weeklySummaryEmails: z.boolean().default(true),
  paymentReminderEmails: z.boolean().default(true),
  productUpdates: z.boolean().default(true),
  betaAnnouncements: z.boolean().default(true),
  securityAlerts: z.boolean().default(true),
  marketingEmails: z.boolean().default(false),
  theme: z.enum(['system', 'light', 'dark']).default('system'),
  accountTimezone: z.string().trim().min(1).max(100).default('UTC'),
  accountCountry: z.string().trim().max(100).default(''),
  workspaceName: z.string().trim().max(200).default(''),
