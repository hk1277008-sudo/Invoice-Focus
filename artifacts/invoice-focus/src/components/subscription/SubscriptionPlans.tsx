...
import { previewUpgrade } from '@/lib/subscription'
...
export function SubscriptionPlans({ initialCycle = 'monthly' }: { initialCycle?: BillingCycle }) {
  ...
  const choose = async (plan: PlanId) => {
    ...
    setBusy(plan)
    try { await previewUpgrade(plan, cycle); toast({ title: `${plan === 'pro' ? 'Pro' : 'Premium'} is ready when you are`, description: 'Payment processing will be connected here in a future release.' }); await refreshSubscription() }
    catch (error) { toast({ title: 'Could not prepare upgrade', description: error instanceof Error ? error.message : 'Please try again.', variant: 'destructive' }) }
  ...
  return <div><div className="mb-8 flex items-center justify-center"><div className="flex rounded-lg border border-border bg-muted/40 p-1"><button onClick={() => setCycle('monthly')} className={`rounded-md px-4 py-2 text-sm font-medium ${cycle === 'monthly' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground'}`}>Monthly</button><button onClick={() => setCycle('yearly')} className={`rounded-md px-4 py-2 text-sm font-medium ${cycle === 'yearly' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground'}`}>Yearly <span className="ml-1 text-xs text-emerald-600">Save 18%</span></button></div></div>
    <div className="grid gap-5 lg:grid-cols-3">{plans.map((plan) => <Card key={plan.id} className={`relative flex flex-col ${plan.popular ? 'border-primary shadow-md shadow-primary/10' : ''}`}>{plan.popular && <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-primary px-3 py-1 text-xs font-semibold text-primary-foreground">Most Popular</div>}<CardHeader><div className="flex items-center justify-between"><CardTitle>{plan.name}</CardTitle>{plan.id === 'premium' && <Crown className="h-5 w-5 text-amber-500" />}</div><CardDescription>{plan.description}</CardDescription><p className="pt-3 font-display text-3xl font-semibold">{cycle === 'yearly' ? plan.yearly : plan.monthly}</p></CardHeader><CardContent className="flex flex-1 flex-col"><ul className="flex-1 space-y-3 text-sm text-muted-foreground">{plan.features.map((feature) => <li key={feature} className="flex gap-2"><Check className="h-4 w-4 shrink-0 text-emerald-500" />{feature}</li>)}</ul><Button className="mt-7 w-full gap-2" variant={plan.id === subscription.plan ? 'secondary' : plan.popular ? 'default' : 'outline'} disabled={plan.id === subscription.plan || busy !== null} onClick={() => choose(plan.id)}>{busy === plan.id ? 'Preparing...' : plan.id === subscription.plan ? 'Current Plan' : <><Sparkles className="h-4 w-4" />Choose {plan.name}</>}</Button></CardContent></Card>)}</div>
  </div>
}