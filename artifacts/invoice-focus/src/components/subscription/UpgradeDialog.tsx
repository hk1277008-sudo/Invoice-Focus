...
export function UpgradeDialog({ open, onOpenChange, feature, description }: { open: boolean; onOpenChange: (open: boolean) => void; feature?: string; description?: string }) {
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-lg">
    ...
    <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button variant="ghost" onClick={() => onOpenChange(false)}>Maybe Later</Button><Button asChild onClick={() => onOpenChange(false)}><Link href="/dashboard/upgrade"><Sparkles className="mr-2 h-4 w-4" />Continue with Pro</Link></Button><Button asChild variant="outline" onClick={() => onOpenChange(false)}><Link href="/dashboard/upgrade?highlight=premium">View Premium</Link></Button></div>
