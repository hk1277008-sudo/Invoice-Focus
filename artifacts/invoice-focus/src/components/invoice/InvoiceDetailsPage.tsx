...
export default function InvoiceDetailsPage({ id }: { id: string }) {
  ...
  const documentMeta = documentTypeMeta(normalizeDocumentType(payload?.documentType))
  const documentLabel = documentMeta.title
  ...
  const defaultSubject = invoice ? `${documentLabel} ${invoice.invoice_number} from Invoice Focus` : ''
  ...
  if (!invoice || !data) return <DashboardLayout><div className="mx-auto max-w-6xl rounded-xl border border-dashed p-12 text-center"><p className="text-muted-foreground">This document could not be found.</p><Button asChild className="mt-4"><Link href="/dashboard">Back to documents</Link></Button></div></DashboardLayout>

  return <DashboardLayout>
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
         <div className="flex items-center gap-3"><Button variant="ghost" size="icon" asChild><Link href="/dashboard" aria-label="Back to documents"><ArrowLeft className="h-4 w-4" /></Link></Button><div><p className="text-sm text-muted-foreground">{documentLabel} details</p><h1 className="font-display text-2xl font-semibold tracking-tight">{invoice.invoice_number}</h1></div><Badge className={`border-0 ${statusStyle[invoice.status]}`}>{invoice.status}</Badge></div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.5fr)_minmax(280px,0.8fr)]">
        <div className="space-y-6">
          <Card><CardContent className="p-6"><div className="flex flex-col gap-6 sm:flex-row sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{documentMeta.billToLabel}</p><p className="mt-2 text-lg font-semibold">{invoice.client || payload?.client?.name || 'Client'}</p><p className="text-sm text-muted-foreground">{invoice.company || payload?.client?.companyName}</p><p className="mt-2 text-sm text-muted-foreground">{payload?.client?.email}</p></div><div className="grid grid-cols-2 gap-x-8 gap-y-3 text-sm sm:text-right"><div><p className="text-xs text-muted-foreground">{documentMeta.issueDateLabel}</p><p className="font-medium">{invoice.issue_date}</p></div><div><p className="text-xs text-muted-foreground">{documentMeta.dueDateLabel}</p><p className="font-medium">{invoice.due_date || '—'}</p></div><div><p className="text-xs text-muted-foreground">{documentMeta.totalLabel}</p><p className="font-semibold text-primary">{money(Number(invoice.total), invoice.currency)}</p></div><div><p className="text-xs text-muted-foreground">Collected</p><p className="font-medium">{money(Number(invoice.amount_paid || 0), invoice.currency)}</p></div></div></div></CardContent></Card>
           <Card><CardHeader><CardTitle className="flex items-center gap-2"><Wallet className="h-5 w-5 text-primary" />{documentMeta.itemsLabel} and totals</CardTitle></CardHeader><CardContent><div className="grid grid-cols-[minmax(0,1fr)_3.5rem_minmax(5.5rem,7.5rem)] border-b pb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground"><span>{documentMeta.itemsLabel}</span><span className="text-right">Qty</span><span className="text-right">Amount</span></div>{(payload?.items || []).map((item) => <div key={item.id} className="grid grid-cols-[minmax(0,1fr)_3.5rem_minmax(5.5rem,7.5rem)] gap-2 border-b py-3 text-sm"><div className="min-w-0"><p className="break-words font-medium">{item.name || 'Item'}</p><p className="break-words text-xs text-muted-foreground">{item.description}</p></div><span className="text-right">{item.quantity}</span><span className="break-all text-right">{money((Number(item.quantity) || 0) * (Number(item.unitPrice) || 0), invoice.currency)}</span></div>)}<div className="ml-auto mt-4 w-full max-w-xs space-y-2 text-sm"><div className="flex justify-between gap-4"><span className="text-muted-foreground">{documentMeta.subtotalLabel}</span><span className="break-all text-right">{money((payload?.items || []).reduce((sum, item) => sum + (Number(item.quantity) || 0) * (Number(item.unitPrice) || 0), 0), invoice.currency)}</span></div><div className="flex justify-between gap-4 border-t pt-3 text-base font-semibold"><span>{documentMeta.totalLabel}</span><span className="break-all text-right">{money(Number(invoice.total), invoice.currency)}</span></div></div></CardContent></Card>
