...
export function buildPrintableInvoiceHTML(invoice: InvoiceData): { html: string; fileName: string } {
  ...
  const calculations = calculateInvoiceTotals(invoice.items, invoice.additional)
  ...
  const itemsHTML = visibleItems
    .map((item) => {
      ...
      const adjustments = showAdjustments
        ...
        : ''
       return `<tr><td><div class="item-name">${escapeHtml(item.name || 'Item')}</div>${item.sku ? `<div class="item-description">SKU: ${escapeHtml(item.sku)}</div>` : ''}${item.description ? `<div class="item-description">${escapeHtml(item.description)}</div>` : ''}</td><td class="num">${values.quantity}</td><td class="num">${formatCurrency(values.unitPrice, currency)}</td>${adjustments}<td class="num">${formatCurrency(values.lineTotal, currency)}</td></tr>`
    })
    .join('')
      ...
      : documentType === 'estimate'
        ...
        : documentType === 'credit-note'
           ...
           : documentType === 'purchase-order'
             ? `<section class="document-section po-section"><div><p class="document-label">Requested Delivery</p><p class="document-value">${escapeHtml(documentDetails.deliveryDate || 'To be confirmed')}</p></div><div><p class="document-label">Authorized By</p><p class="document-value">${escapeHtml(documentDetails.authorizedBy || '—')}</p></div><div><p class="document-label">Authorization Date</p><p class="document-value">${escapeHtml(documentDetails.authorizationDate || '—')}</p></div>${documentDetails.deliveryInstructions ? `<div class="signature"><p class="document-label">Delivery Instructions</p><p class="document-copy">${escapeHtml(documentDetails.deliveryInstructions)}</p></div>` : ''}</section>`
          : ''
...
<main class="invoice">${family === 'enterprise' ? '<div class="topbar"></div>' : family === 'professional' ? '<div class="rail"></div>' : '<div class="orb"></div>'}
  <header class="header"><div class="header-left">${invoice.business.logo ? `<img class="logo" src="${escapeHtml(invoice.business.logo)}" alt="${escapeHtml(invoice.business.name || 'Business')} logo">` : ''}<p class="business-name">${escapeHtml(invoice.business.name || 'Business Name')}</p>${invoice.business.contactPerson ? `<p class="muted">${escapeHtml(invoice.business.contactPerson)}</p>` : ''}${invoice.business.address ? `<p class="muted">${escapeHtml(invoice.business.address).replace(/\n/g, '<br>')}</p>` : ''}<div style="margin-top:6px">${invoice.business.email ? `<p class="muted">${escapeHtml(invoice.business.email)}</p>` : ''}${invoice.business.phone ? `<p class="muted">${escapeHtml(invoice.business.phone)}</p>` : ''}${invoice.business.website ? `<p class="muted">${escapeHtml(invoice.business.website)}</p>` : ''}</div>${invoice.business.taxId ? `<p class="muted" style="margin-top:8px">Tax ID: ${escapeHtml(invoice.business.taxId)}</p>` : ''}</div><div class="header-right"><p class="meta-label">${escapeHtml(documentMeta.numberLabel)}</p><p class="business-name" style="font-size:${presentation.titleStyle === 'compact' ? '19px' : '24px'}">${escapeHtml(documentMeta.title)}</p>${invoice.details.number ? `<p class="meta-value">${escapeHtml(invoice.details.number)}</p>` : ''}${documentType === 'receipt' ? '<span class="status">PAID</span>' : invoice.details.status ? `<span class="status">${escapeHtml(invoice.details.status)}</span>` : ''}</div></header>
   ...
   <div class="totals"><div class="totals-row"><span>${documentMeta.subtotalLabel}</span><span>${formatCurrency(calculations.subtotal, currency)}</span></div>${calculations.discount > 0 ? `<div class="totals-row"><span>Discount</span><span>-${formatCurrency(calculations.discount, currency)}</span></div>` : ''}${calculations.tax > 0 ? `<div class="totals-row"><span>${documentMeta.taxLabel}</span><span>${formatCurrency(calculations.tax, currency)}</span></div>` : ''}${calculations.shipping > 0 ? `<div class="totals-row"><span>Shipping / Handling</span><span>${formatCurrency(calculations.shipping, currency)}</span></div>` : ''}<div class="totals-row total"><span>${documentMeta.totalLabel}</span><span style="color:${dark ? '#fff' : presentation.primaryColor}">${formatCurrency(calculations.grandTotal, currency)}</span></div></div>
