    ...
    <DashboardLayout>
      <div className="space-y-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div><h1 className="font-display text-2xl font-semibold tracking-tight">Clients</h1><p className="mt-1 text-sm text-muted-foreground">Keep client details organized and ready for every invoice.</p></div>
        ...
        {loading ? <div className="rounded-xl border border-border bg-card p-12 text-center text-sm text-muted-foreground">Loading clients...</div> : clients.length === 0 ? (
          <div className="flex min-h-[320px] flex-col items-center justify-center rounded-xl border border-dashed border-border bg-card p-8 text-center"><UserRound className="h-9 w-9 text-muted-foreground/50" /><h2 className="mt-4 font-medium">No Clients Yet</h2><p className="mt-1 max-w-sm text-sm text-muted-foreground">Add your first client to quickly reuse their information on future invoices.</p><Button onClick={openCreate} className="mt-5">Add Your First Client</Button></div>
        ) : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{clients.map((client) => (
          <Card key={client.id} className="cursor-pointer transition-shadow hover:shadow-md" onClick={() => openDetail(client)}><CardContent className="p-5">
      ...
      </div><DialogFooter><Button variant="outline" onClick={() => setDialogOpen(false)}>Cancel</Button><Button onClick={save} disabled={saving}>{saving ? 'Saving...' : editing ? 'Save Changes' : 'Add Client'}</Button></DialogFooter></DialogContent></Dialog>
