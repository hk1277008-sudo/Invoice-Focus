...
export default function SettingsPage() {
  ...
  const update = (key: keyof UserSettings, value: string | number | boolean) => setSettings((current) => ({ ...current, [key]: value }))
  const save = async () => {
...
function SubscriptionTab() {
  return <div className="grid gap-6 lg:grid-cols-[1.1fr_1fr]"><Card className="border-primary/30"><CardHeader><div className="flex items-center justify-between"><CardTitle>Free Plan</CardTitle><span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">Current Plan</span></div><CardDescription>Simple invoicing for getting started.</CardDescription></CardHeader><CardContent><p className="font-display text-3xl font-semibold">15 <span className="text-base font-normal text-muted-foreground">invoices per month</span></p><div className="mt-5 h-2 rounded-full bg-muted"><div className="h-2 w-1/3 rounded-full bg-primary" /></div><p className="mt-2 text-sm text-muted-foreground">Remaining invoices this month: 10</p><ul className="mt-6 space-y-3 text-sm text-muted-foreground"><li className="flex gap-2"><Check className="h-4 w-4 text-emerald-500" />Professional invoice generator</li><li className="flex gap-2"><Check className="h-4 w-4 text-emerald-500" />Saved clients and invoice history</li><li className="flex gap-2"><Check className="h-4 w-4 text-emerald-500" />PDF export and business dashboard</li></ul></CardContent></Card><Card><CardHeader><CardTitle>Upgrade Your Workspace</CardTitle><CardDescription>Unlock more room to grow. Payment processing is not connected yet.</CardDescription></CardHeader><CardContent className="space-y-3"><div className="rounded-lg border border-border p-4"><p className="font-semibold">Pro</p><p className="mt-1 text-sm text-muted-foreground">Unlimited invoices, advanced reminders, and richer reporting.</p><Button variant="outline" className="mt-4 w-full" onClick={() => window.alert('Pro upgrades are coming soon.')}>Upgrade to Pro</Button></div><div className="rounded-lg border border-border p-4"><p className="font-semibold">Premium</p><p className="mt-1 text-sm text-muted-foreground">Team workflows, priority support, and advanced controls.</p><Button variant="outline" className="mt-4 w-full" onClick={() => window.alert('Premium upgrades are coming soon.')}>Upgrade to Premium</Button></div></CardContent></Card></div>
}
...
function PrivacyTab({ toast }: { toast: ReturnType<typeof useToast>['toast'] }) {
  ...
  const exportAll = async () => { try { await download('invoicefocus-account-data.json', await exportSettingsData()); toast({ title: 'Account data exported' }) } catch (error) { toast({ title: 'Export failed', description: error instanceof Error ? error.message : 'Please try again.', variant: 'destructive' }) } }
  ...
  return <div className="space-y-6"><Card><CardHeader><CardTitle>Export Your Data</CardTitle><CardDescription>Download a portable copy of your InvoiceFocus workspace data.</CardDescription></CardHeader><CardContent className="flex flex-col gap-3 sm:flex-row"><Button variant="outline" className="gap-2" onClick={exportAll}><Download className="h-4 w-4" />Export Account Data</Button><Button variant="outline" className="gap-2" onClick={async () => { try { const data = await exportSettingsData(); await download('invoicefocus-clients.json', data.clients); toast({ title: 'Client data downloaded' }) } catch (error) { toast({ title: 'Export failed', description: error instanceof Error ? error.message : 'Please try again.', variant: 'destructive' }) } }}>Download Client Data</Button><Button variant="outline" className="gap-2" onClick={async () => { try { const data = await exportSettingsData(); await download('invoicefocus-invoices.json', data.invoices); toast({ title: 'Invoice data downloaded' }) } catch (error) { toast({ title: 'Export failed', description: error instanceof Error ? error.message : 'Please try again.', variant: 'destructive' }) } }}>Download Invoice Data</Button></CardContent></Card><Card className="border-destructive/30"><CardHeader><CardTitle className="flex items-center gap-2 text-destructive"><AlertTriangle className="h-5 w-5" />Delete Account</CardTitle><CardDescription>Deleting your account permanently removes all invoices, clients, settings, and business data. This cannot be undone.</CardDescription></CardHeader><CardContent className="space-y-4"><Field label="Type DELETE MY ACCOUNT to confirm" value={confirm} onChange={setConfirm} placeholder="DELETE MY ACCOUNT" /><Button variant="destructive" disabled={confirm !== 'DELETE MY ACCOUNT' || deleting} onClick={deleteUser}>{deleting ? 'Deleting Account...' : 'Request Account Deletion'}</Button></CardContent></Card></div>
}