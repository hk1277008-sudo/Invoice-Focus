...
import { documentTypeLabel, normalizeDocumentType } from '@/components/invoice/document-types'
...
export default function SharedInvoicePage() {
  ...
  if (!invoice || !currency) return <div className="flex min-h-svh items-center justify-center bg-muted/30 text-sm text-muted-foreground">Loading invoice…</div>
   const calculations = calculateInvoiceTotals(invoice.payload.items, invoice.payload.additional)
   const documentLabel = documentTypeLabel(normalizeDocumentType(invoice.payload.documentType))
   return <div className="min-h-svh bg-muted/30 py-6 sm:py-10"><main className="mx-auto max-w-5xl px-4 sm:px-6"><header className="mb-6 flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">{documentLabel}</p><h1 className="text-xl font-semibold">{invoice.invoice_number}</h1><p className="text-sm text-muted-foreground">Status: {invoice.status} · {invoice.due_date ? `Due ${invoice.due_date}` : 'No due date'}</p></div><div className="flex gap-2 print:hidden"><Button variant="outline" onClick={() => window.print()}><Printer className="mr-2 h-4 w-4" />Print</Button><Button onClick={() => downloadPDF(invoice.payload)}><Download className="mr-2 h-4 w-4" />Download PDF</Button></div></header><div className="print:shadow-none"><InvoicePreview invoice={invoice.payload} currency={currency} calculations={calculations} hasAnyData /></div><p className="mt-6 text-center text-xs text-muted-foreground print:hidden">Secure document portal · Shared by Invoice Focus.</p></main></div>
}